...
- **Campaign Model:** Automated 30-day trading campaigns with lifecycle management, investor profiles (Conservative, Moderate, Aggressive) defining risk parameters, a 5-step wizard with inline portfolio creation, daily compounding, and a -10% drawdown circuit breaker.
- **Campaign Wizard UX:** The 5-step wizard includes inline portfolio creation in Step 3 for users without portfolios. Live portfolio option is automatically disabled when Kraken credentials are not configured, preventing validation errors.
